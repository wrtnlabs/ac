...
ac-managed                 LIVE: optional backend-authoritative layer above the runtime — durable
                           opaque submissions, guarded pending reorder, atomic per-session claim,
                           lossless pending-to-active steer promotion with ordered settlement proof,
                           start-or-queue, cross-mode direct-run leases, sequential drain,
                           idempotent input-commit phase, and restart recovery; host injects
                           runner/event projection, credentials never persist
...
ac-skills                  LIVE: SKILL.md skills mirroring the codex-rs architecture (studied
                           ...
                           the model reads companion files itself at the listed paths. Real YAML
                           frontmatter preserves nested metadata, sequences, flow collections, and
                           block scalars (anchors/aliases/tags rejected). Discovery offers a bounded
                           recursive/canonical-path mode (directory-name fallback, duplicate names
...
ac-store                   LIVE: rusqlite session+message store — opaque or caller-adopted ids,
                           ...
                           durable opaque managed-submission records + one active-run lease;
                           pairs with Session::resume for reload recovery — phase 3
...
- **`deny_paths` nested inside a write root is a macOS-only guarantee.** Seatbelt is last-match-wins, so a deny beats an enclosing allow (`macos.rs` emits the write allows first, then the denies — that order is load-bearing and pinned by `writing_to_a_denied_path_inside_a_write_root_is_blocked`). Landlock v1 is ALLOW-ONLY and has no deny rule, so on Linux a denied path *inside* a granted root is not carved out; the kit's own default secret set is safe only because it lives OUTSIDE every granted root. A host whose policy denies something nested in a write root (a repo's `.git/hooks` while granting the repo) gets that boundary on macOS and silently does not on Linux. The honest fix is to grant the complement — enumerate the children of the root and grant all but the denied ones — which landlock can express because it grants by `PathFd`; the catch is that a path created later is not covered, so it needs re-derivation per call. Until then the asymmetry is documented in `linux.rs`, the README's known-limits, and here.
- **Egress / SSRF:** `shell` reaches whatever the installed sandbox network mode permits. `fetch` is a separate in-process mechanism: unrestricted by default, or restricted to host-approved HTTP(S) URL origins with every redirect rechecked. That URL policy limits authority exposed to the model, but it is not kernel egress isolation and must not be described as one; full DNS/IP/proxy containment remains `ac-sandbox` v2. See docs/ac-sandbox.md and docs/ac-tools.md §4.3.
- **Steering and managed next-turn submissions are distinct, and both are live.** `ac-runtime` owns the in-turn steer queue and step-boundary drain ([docs/ac-queue-steer.md](docs/ac-queue-steer.md)); `ac-managed` owns durable next-turn admission, guarded pending reorder, atomic ordered claim, lossless pending-to-active promotion through `steering → delivered → ordered settlement ACK`, the shared direct-run lease/publication fence, sequential drain, and restart reconciliation ([docs/ac-managed-submissions.md](docs/ac-managed-submissions.md)). A client never checks cached status to choose start versus enqueue, and a host using `ManagedRuns` never bypasses it with raw store lease calls. Fork/rewind and compaction are also implemented through `ac-rollout`/`ac-runtime`; the documents remain the contracts.
